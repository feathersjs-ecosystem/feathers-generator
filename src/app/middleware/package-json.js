...
export default function (options) {
  return function packageJSON (files, metalsmith, done) {
    ...
    if (babel) {
      ...
      template.scripts.mocha = `mocha {test/,server/**/*.test.js,server/**/**/*.test.js} --compilers js:babel-core/register --recursive`;
    } else {
      template.scripts.start = `node index.js`;
      template.scripts.mocha = `mocha {test/,server/**/*.test.js,server/**/**/*.test.js} --recursive`;
    }
